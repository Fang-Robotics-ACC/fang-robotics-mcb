...
4. Modularity - modifying a line of code should not necessarily cause fears of catastrophic damage.
...
1. If it is broken, fix it now. (We allow minor formatting errors to be bundled with any type of commit - only minor ones however such as extra lines or spaces).
...
10. A programmer who needs to always look something up every other line or get another programmer (or AI especially) to write it is always less skilled.
...
A major version simply means that we are implementing a major addition i.e. CV, more robots
or the start of a new competition cycle. Minor versions will keep incrementing.
The scope of a major version/minor version must be defined in the release branch to prevent eternal release syndrome
...
* Releases must be merged sequentially, if the current release branch is release/v4.8.0,
	v4.9.0 cannot be merged regardless. It is recommended to the current lead dev that
	...
	if it works
* A release is final. If a hotfix must be done, it will be included in a new release with the 3rd digit incremeted
	i.e. if the most recent tag is v4.8.1, the next tag is v4.8.2. 
	If the next release has begun work, a hotfix will be merged to main, tagged, then merged
...
release/*:
* Only commits which compile on test, compile on build, and pass all unit tests are accepted.
feat/*:
* When these are ready for a pull request, they are pushed onto the github repo, they can **never** be
	merged directly into main.	
hotfix/*:
* These must be verified with hardware, merged directly to main, tagged
...
We decided to indent namespaces because it looks cleaner.
...
Use mixedLowerCase. `Class::staticFunction()`

### Interfaces
#### Interface Naming Conventions. 
An interface (purely abstract class with virtual functions), shall be appended with I.
